...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($typ_konta === 'praktykant') {
        ...
        $_SESSION['messageLogin'] = "Niepoprawny typ konta.";
        exit();
    }
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        if (password_verify($haslo, $row['haslo'])) {
            ...
            if ($typ_konta === 'praktykant') {
                ...
                $_SESSION['user_firma'] = $row['firma'];
                header("Location: index.php");
                // header("Location: panelPracodawcy.php?user=".$row['firma'].'.'.$row['firma']);
            }
            exit();
        } else {
            $_SESSION['messageLogin'] = "Niepoprawne dane logowania.";
        }
    } else {
        $_SESSION['messageLogin'] = "Użytkownik o podanym adresie email nie istnieje.";
    }
}
?>
<!DOCTYPE html>
